 ...
 *
 */
public class OurAgent extends Player {
	 ...
	 * which in this example is done in the init() method of the OurAgent.  
	 * 
	...
	//CONSTRUCTOR
	OurAgent(String name, int finalYear, String logPath, int gameServerPort, int negoServerPort){
		super(logPath);
	...
	public void init() {
		...
		if(this.negoClient.getStatus() == STATUS.READY){
			logger.logln("OurAgent.init() " + this.me.getName() + " Connection with negotiator correctly established. Ready to start negotiating!");
		}else{
			logger.logln("OurAgent.init() " + this.me.getName() + " connection failed! " + this.negoClient.getStatus(), true);
		}
		logger.writeToFile();
	...
	public List<Order> play() {
		...
		if(game.getPhase() == Phase.SPR || game.getPhase() == Phase.FAL){
			...
			//OurAgent does not negotiate during retreat phases, but you can implement your agent to do so.
			...
			//OurAgent does not negotiate during build phases, but you can implement your agent to do so.
		...
		while(System.currentTimeMillis() < negotiationDeadline){
			...
			while(negoClient.hasMessage()){
				...
				if(receivedMessage.getPerformative().equals(DiplomacyNegoClient.ACCEPT)){
					...
					this.logger.logln("OurAgent.negotiate() Received acceptance from " + receivedMessage.getSender() + ": " + acceptedProposal);
					...
					this.logger.logln("OurAgent.negotiate() Received proposal: " + receivedProposal);
					...
					if(!outDated && consistencyReport == null){
						...
						if(random.nextInt(2) == 0){ // accept with 50% probability.
							...
							this.logger.logln("OurAgent.negotiate()  Accepting: " + receivedProposal);
						}
					}
					...
					//this.logger.logln();
					this.logger.logln("OurAgent.negotiate() RECEIVED CONFIRMATION OF: " + confirmedProposal);
			...
			if(newDealToPropose != null){
				...
				try {
					this.logger.logln("OurAgent.negotiate() Proposing: " + newDealToPropose);
	...
	public BasicDeal generateRandomDeal(){
		...
		for(int i=0; i<3; i++){
			...
			//The OurAgent only generates deals for the current year and phase. 
